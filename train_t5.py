...
class t5_dataset_query(Dataset):
    def __init__(self, train_path, tokenizer, max_length=300,is_eval = False):
        ...
        if(self.is_valid == False):
            ...
            self.input_ids = [torch.tensor(self.tokenizer(self.prepare_prompt(self.dataset[i]['context'],self.dataset[i]['entity']))['input_ids']) for i in range(0,self.n_eval)]
            self.atten_masks = [torch.tensor(self.tokenizer(self.prepare_prompt(self.dataset[i]['context'],self.dataset[i]['entity']))['attention_mask']) for i in range(0,self.n_eval)]
            self.labels = [torch.tensor(self.tokenizer(self.dataset[i]['query_gen'])["input_ids"]) for i in range(0,self.n_eval)]
    def __len__(self):
        if(self.is_valid == False):
            return len(self.dataset) - 1000
        else:
...
if __name__ == "__main__":
    ...
    ds_train = t5_dataset_query("./gpt_query.json",tokenizer)
    ds_eval = t5_dataset_query("./gpt_query.json",tokenizer,is_eval= True)
    out_dir = "./out_new"
