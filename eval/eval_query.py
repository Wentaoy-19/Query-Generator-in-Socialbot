...
def query_inference(context: str, model,entity,cosmo):
    input_text = prepare_prompt_query(context,entity,cosmo)
    # print(input_text)
...
def evaluate_query(idx:int,dataset,model,cosmo):
    ...
    # inference
    query = query_inference(context,model,entity,cosmo)
    dataset[idx]['predict_query'] = query
    score = rouge.get_scores(query, gt_query)[0]['rouge-l']['f']
...
def evaluate_query_main(dataset_path,out_path,model_path,cosmo):
    model = AutoModelForSeq2SeqLM.from_pretrained(model_path).cuda()
    fp = open(dataset_path)
    dataset = json.load(fp)
    length = 1000
    score = 0
    for i in tqdm(range(length)):
        _score = evaluate_query(i,dataset,model,cosmo)
        score += _score
    score/= length
    print("Avg Rouge Score:", score)
    with open(out_path,'w') as fpout:
        json.dump(dataset[:length],fpout)
    return score
...
if __name__ == "__main__":
    ...
    parser.add_argument('--eval_set',type=str,default='../dataset/train_query.json')
    ...
    evaluate_query_main(args.eval_set,args.out_path,args.model_path,cosmo)
